...
void write_string(struct string *str) {
  fprintf(stderr, "\x1B[35mstr @ %p\tdata @ %p\x1B[0m\n", str, str->data);
...
void showstr(struct string *s) {
	fprintf(stderr, "struct string { length = %d, capacity = %d, data = %p }\n",
		s->length, s->capacity, s->data);
}
